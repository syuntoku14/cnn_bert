...
class BertDataset(Dataset):
    def __init__(self, device):
        ...
        if os.path.isfile("bert_large_model.pkl"):
            with open('bert_large_model.pkl','rb') as bert_model_file:
                print('load model')
                self.bert = pickle.load(bert_model_file)
        else:
            self.bert = BertModel.from_pretrained("bert-large-uncased").to(device)
            with open('bert_large_model.pkl','wb') as bert_model_file:
                pickle.dump(self.bert,bert_model_file)
...
class CNN_LM(nn.Module):
    def __init__(self, char_vocab_size, char_len, embed_dim, chan_size, hid_size, bert_hid_size):
        ...
        convs[-1] = Conv1dBlockBN(chan_size, BERT_EMBED_DIM , 2, stride=1)
...
def find_closest_words(index, inputs, outputs, targets):
    ...
    #words = inputs[loss(targets,outputs[index]).topk(20, largest=False).indices]
...
def find_closest_words_amond_bert_embeddings(index, inputs, outputs, targets):
    #loss = -nn.CosineSimilarity(dim=1,eps=1e-6)
    #words = inputs[loss(targets,outputs[index]).topk(20, largest=False).indices]
    words = inputs[torch.norm(targets - targets[index], dim=1).topk(20, largest=False).indices]
    return list(map(to_word, words.to(torch.device('cpu')).tolist()))

def print_closest_words(index, inputs, outputs, targets):
    print('target word: %s' % (to_word(inputs[index])))
    print('closest[bert vs our model]: %s' % (find_closest_words(index, inputs, outputs, targets)))
    print('closest[bert vs bert]: %s' % (find_closest_words_amond_bert_embeddings(index, inputs, outputs, targets)))
...
if __name__ == "__main__":
    ...
    if os.path.isfile("bert_dataset.pkl"):
        with open('bert_dataset.pkl','rb') as bert_dataset_file:
            print('load dataset from pickle')
            dataset = pickle.load(bert_dataset_file)
    else:
        dataset = BertDataset(device)
        with open('bert_dataset.pkl','wb') as bert_dataset_file:
            pickle.dump(dataset,bert_dataset_file)
    # test train
    ...
    if args.skip_train:
        ...
        torch.save(model.state_dict(), './data/bert_large_embed.ckpt')

...
class Interpreter(nn.Module):
    ...
    def forward(self):
        ...
        loss = (s_tilde - s) ** 2
        if self.regular is not None:
            loss = torch.mean(loss / (self.regular ** 2)+0.01)
        else:
            loss = torch.mean(loss) / (torch.mean(s ** 2) + 0.01)
        #print(torch.log(ratios+0.001))
        #print(ratios+0.001)
        #print(torch.mean(torch.log(ratios+0.001)))


        return loss - torch.mean(torch.log(ratios+0.01)) * self.rate
